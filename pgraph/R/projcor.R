# Modified from projcov.R by Kiran Karra (kiran.karra@gmail.com) to return the 
# distance correlation of the residuals, which should be a rough measure of conditional
# dependence.
...
projcor <- function(x, y, b, method = c("lasso","sam"), one.SE = TRUE, refit = TRUE, randSeed = 0){
  ...
  projcor = energy::dcor(xeps,yeps)

return(list=list(projcor.statistic=projcor, xeps=xeps,yeps=yeps))
}
